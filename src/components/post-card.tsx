...
// HTML 엔티티와 불필요한 공백을 제거하는 함수 추가
function cleanExcerpt(excerpt: string): string {
    return excerpt
        .replace(/&nbsp;/g, ' ')          // &nbsp; 제거
        .replace(/&[a-zA-Z0-9#]+;/g, ' ') // 기타 HTML 엔티티 제거
        .replace(/\s+/g, ' ')             // 연속 공백 제거
        .trim();
}

export function PostCard({post}: PostCardProps) {
    const rawExcerpt = post.excerpt || stripHtmlTags(post.content).substring(0, 150) + "...";
    const displayExcerpt = cleanExcerpt(rawExcerpt);
  ...
  `;
    ...
    return (
        <div className={containerClass}>
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-4 line-clamp-3">{displayExcerpt}</p>
            ...
            {post.tags && post.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    {post.tags.length > 3 && <span className="text-xs text-gray-500 dark:text-gray-400 px-2 py-1">+{post.tags.length - 3}개 더</span>}
                </div>
            )}
